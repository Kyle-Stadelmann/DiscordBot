...
class StartCommand extends Command {
	public async run(msg: Message): Promise<boolean> {
        ...
        await msg.reply({components: [msgActionRow], content: `${displayName} has initiated a ${role.name}. Number of players ${1}/${maxPlayers}`})
    ...
    private async validateCmd(msg: Message): Promise<boolean> {
        ...
        if (!playersPerGameMap.has(roleId)) {
            await sendErrorMessage(msg.channel, `Invalid game role specified. Currently supported roles: ${supportedRolesStr}`);
            return false;
        }
...
async function joinQueue(interaction: ButtonInteraction, guildId: string, userId: string, roleId: string) {
    ...
    if (players.length === maxPlayers) {
        await interaction.reply({content: "Sorry, the max number of players for this queue has been reached. Please wait or ask to switch with another player.", ephemeral: true})
    } else {
        playersContainer.addPlayer(guildId, userId, roleId);

        players = playersContainer.getPlayers(guildId, roleId);
        await interaction.reply(`${displayName} has joined the ${roleName} queue. Number of players ${players.length}/${maxPlayers}`);
        if (players.length === maxPlayers) {
    ...
    try {
        ...
        if (customId.startsWith(joinBtnId)) {
            ...
            if (playersContainer.hasPlayer(guildId, userId, roleId)) {
                ...
                await interaction.reply(`${displayName} left the ${roleName} queue. Number of players ${numPlayers}/${maxPlayers}`);
            } else {
