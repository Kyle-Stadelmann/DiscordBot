...
class IdeaButton {
	...
	async submitBtn(interaction: ButtonInteraction) {
		...
		const modal = new Modal().setTitle("Submit an Idea").setCustomId("idea-modal");
		...
		const row1 = new MessageActionRow<ModalActionRowComponent>().addComponents(typeInputComponent);
		const row2 = new MessageActionRow<ModalActionRowComponent>().addComponents(ideaInputComponent);
