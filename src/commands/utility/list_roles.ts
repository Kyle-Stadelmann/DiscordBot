...
class ListRolesCommand extends Command {
	public async run(msg: Message): Promise<boolean> {
		...
		// Grab the main roles
		let mainRoles = allRoles.filter((role) => role.permissions.bitfield > 0 && !role.managed && role.name !== "@everyone");
	
		// Sort the main roles
		mainRoles = mainRoles.sort((a, b) => b.comparePositionTo(a));
	
		// Grab the player roles
		let playerRoles = allRoles.filter((role) => role.permissions.bitfield === 0n && !role.managed && role.mentionable);
