...
export const activateCooldown = (bot, cmdName, member) => {
    if (!member) return;

    // If command doesn't have a cooldown, return early
    if (!Constants.cooldownTimes.has(cmdName)) return;
    ...
    bot.util.writeDisk(bot, bot.cooldowns, Constants.COOLDOWN_JSON_LOC);
}
...
// are on cooldown.
export const endCooldown = (bot, cmdName, member) => {
...
export const isOnCooldown = (bot, cmdName, member) => {
    // If member doesn't exist... no cooldown
    ...
    let coolTime = Constants.cooldownTimes.get(cmdName);
...
export const readDiskCooldowns = (bot) => {
